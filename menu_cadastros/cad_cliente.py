...
class TelaCadastroCliente(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        validator = QtGui.QIntValidator(0, 123456, self.line_Num)
    ...
    def manipula_dados_tabela(self):
        try:
            ...
            cursor.execute(f"select id, COALESCE(data_criacao, ''), registro, razao, COALESCE(venda, '') "
                           f"from clientes "
                           f"where id <> 0 "
                           f"order by razao;")
            ...
            if select_numero:
                lanca_tabela(self.table_Lista, select_numero)
    ...
    def procura_palavra(self):
        try:
            ...
            if not palavra_consulta:
                ...
                cursor = conecta.cursor()
                cursor.execute(f"SELECT DISTINCT id, COALESCE(data_criacao, ''), registro, razao, COALESCE(venda, '') "
                               f"FROM clientes "
