...
from forms.tela_cad_conjunto import *
...
class TelaCadastroConjunto(QMainWindow, Ui_MainWindow):
    ...
    def manipula_dados_tabela(self):
        try:
            ...
            cursor.execute(f"select id, data_criacao, conjunto "
                           f"from conjuntos "
                           f"where id <> 0 "
                           f"order by conjunto;")
    ...
    def lanca_numero_id(self):
        try:
            definir_proximo_registro(self.line_Num, "id", "conjuntos")
    ...
    def procura_palavra(self):
        try:
            ...
            if not palavra_consulta:
                ...
                cursor = conecta.cursor()
                cursor.execute(f"SELECT DISTINCT id, data_criacao, conjunto "
                               f"FROM conjuntos "
                               f"WHERE conjunto LIKE '%{palavra_maiuscula}%' and id <> 0 "
                               f"ORDER BY conjunto;")
    ...
    def excluir_cadastro(self):
        try:
            ...
            if not num_id:
                ...
                cursor = conecta.cursor()
                cursor.execute(f"select id, data_criacao, conjunto "
                               f"from conjuntos where id = {num_id} and id <> 0 ;")
                conjunto = cursor.fetchall()

                cursor = conecta.cursor()
                cursor.execute(f"SELECT * "
                               f"FROM produto "
                               f"WHERE conjunto = {num_id};")
                produtos = cursor.fetchall()

                if not conjunto:
                    self.mensagem_alerta(f'O cadastro de Conjunto Nº {num_id} não existe!')
                elif produtos:
                    self.mensagem_alerta(f'O cadastro de Conjunto Nº {num_id} não pode ser excluído, '
                                         f'pois existem vinculos com produtos!')
                else:
                    msg = f'Tem certeza que deseja excluir o Conjunto {nome}?'
                    if self.pergunta_confirmacao(msg):
                        cursor = conecta.cursor()
                        cursor.execute(f"DELETE FROM conjuntos WHERE id = {num_id};")

                        conecta.commit()

                        self.mensagem_alerta(f"Cadastro do Conjunto {nome} foi excluído com Sucesso!")
    ...
    def salvar_dados(self):
        try:
            ...
            cursor = conecta.cursor()
            cursor.execute(f"select id, data_criacao, conjunto "
                           f"from conjuntos where id = {num_id};")
            ...
            if conjuntos:
                ...
                if descr_sem_acentos != conj_b:
                    campos_atualizados.append(f"conjunto = '{descr_sem_acentos}'")

                if campos_atualizados:
                    msg = f'Deseja realmente atualizar o cadastro do Conjunto?'
                    if self.pergunta_confirmacao(msg):
                        campos_update = ", ".join(campos_atualizados)

                        cursor.execute(f"UPDATE conjuntos SET {campos_update} "
                                       f"WHERE id = {num_id_b};")

                        conecta.commit()

                        msg = f'O cadastro do Conjunto {descr_sem_acentos} foi atualizado com sucesso!'
                        self.mensagem_alerta(msg)

            else:
                msg = f'Deseja realmente cadastrar este Conjunto?'
                if self.pergunta_confirmacao(msg):
                    cursor = conecta.cursor()
                    cursor.execute(f"Insert into conjuntos (ID, CONJUNTO) "
                                   f"values (GEN_ID(GEN_CONJUNTOS_ID,1), '{descr_sem_acentos}');")

                    conecta.commit()

                    msg = f'O cadastro do Conjunto {descr_sem_acentos} foi criado com sucesso!'
                    self.mensagem_alerta(msg)
