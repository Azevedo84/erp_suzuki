...
class TelaCadastroServicoInterno(QMainWindow, Ui_MainWindow):
    ...
    def manipula_dados_tabela(self):
        try:
            ...
            cursor.execute(f"select id, data_criacao, descricao "
    ...
    def lanca_numero_id(self):
        try:
            definir_proximo_registro(self.line_Num, "id", "SERVICO_INTERNO")
    ...
    def procura_palavra(self):
        try:
            ...
            if not palavra_consulta:
                ...
                cursor = conecta.cursor()
                cursor.execute(f"SELECT DISTINCT id, data_criacao, descricao "
                               f"FROM SERVICO_INTERNO "
    ...
    def excluir_cadastro(self):
        try:
            ...
            if not num_id:
                ...
                cursor = conecta.cursor()
                cursor.execute(f"select id, data_criacao, descricao "
                               f"from SERVICO_INTERNO where id = {num_id} and id <> 0 ;")
                conjunto = cursor.fetchall()

                cursor = conecta.cursor()
                cursor.execute(f"SELECT * "
                               f"FROM produto "
                               f"WHERE ID_SERVICO_INTERNO = {num_id};")
    ...
    def salvar_dados(self):
        try:
            ...
            if conjuntos:
                ...
                if self.pergunta_confirmacao(msg):
                    cursor = conecta.cursor()
                    cursor.execute(f"Insert into SERVICO_INTERNO (ID, DESCRICAO) "
                                   f"values (GEN_ID(GEN_SERVICO_INTERNO_ID,1), '{descr_sem_acentos}');")

                    conecta.commit()

                    msg = f'O cadastro do Serviço Interno {descr_sem_acentos} foi criado com sucesso!'
                    self.mensagem_alerta(msg)
