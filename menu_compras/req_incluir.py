            ...
            if (event.type() == QtCore.QEvent.MouseButtonDblClick and event.buttons() == QtCore.Qt.LeftButton
                ...
                if num_tipo:
                    ...
                    cursor.execute(f"SELECT prodreq.mestre, prod.codigo, "
                                   ...
                                   f"WHEN prod.embalagem = 'SER' THEN prodreq.referencia "
                                   f"ELSE prod.obs END) as REFERENCIA, "
                                   f"prod.unidade, prodreq.quantidade, "
                                   f"(select MAX(prc.unitario) from produtoordemcompra prc "
                                   f"INNER JOIN ordemcompra ON (prc.mestre = ordemcompra.id) "
                                   f"WHERE ordemcompra.entradasaida = 'E' AND prc.produto = prodreq.produto AND "
                                   ...
                                   f"where ppc.produto = prodreq.produto AND ordemcompra.entradasaida = 'E')), "
                                   f"(select MAX(prc.ipi) from produtoordemcompra prc "
                                   f"INNER JOIN ordemcompra ON (prc.mestre = ordemcompra.id) "
                                   f"WHERE ordemcompra.entradasaida = 'E' AND prc.produto = prodreq.produto AND "
                                   f"prc.dataentrega = (select MAX(ppc.dataentrega) from produtoordemcompra ppc "
                                   f"INNER JOIN ordemcompra ON (ppc.mestre = ordemcompra.id) "
                                   f"where ppc.produto = prodreq.produto AND ordemcompra.entradasaida = 'E')) "
                                   f"AS EPI, "
                                   f"(select MAX(fornecedores.razao) from produtoordemcompra prc "
                                   f"INNER JOIN ordemcompra ON (prc.mestre = ordemcompra.id) "
                                   ...
                                   f"INNER JOIN ordemcompra ON (ppc.mestre = ordemcompra.id) "
                                   f"where ppc.produto = prodreq.produto AND ordemcompra.entradasaida = 'E') "
                                   f"GROUP BY fornecedores.id) AS Fornecedor, prodreq.destino, req.nome_pc "
                                   ...
                                   f"WHERE prodreq.status = 'A' AND prod.tipomaterial = {num_tipo} "
