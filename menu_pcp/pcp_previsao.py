...
class TelaPcpPrevisao(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.btn_GeraExcel.clicked.connect(self.excel)

        self.definir_predefinidos()
        self.configura_lineedit()
    ...
    def definir_predefinidos(self):
        try:
            ...
            self.date_Inicio.setDate(data_ini)

            self.line_Func.setText("5")
            self.line_HorasDia.setText("8")
    ...
    def configura_lineedit(self):
        try:
            validator = QtGui.QIntValidator(0, 123456, self.line_Func)
            locale = QtCore.QLocale("pt_BR")
            validator.setLocale(locale)
            self.line_Func.setValidator(validator)

            validator = QtGui.QIntValidator(0, 123456, self.line_HorasDia)
            locale = QtCore.QLocale("pt_BR")
            validator.setLocale(locale)
            self.line_HorasDia.setValidator(validator)
    ...
    def calculo_5_recebe_dados(self, estrutura_ord):
        try:
            ...
            data_ini = self.date_Inicio.text()
            formato = "%d/%m/%Y"
            ...
            for i in estrutura_ord:
                ...
                msg1 = "Calculando Tempos Produção..."
                msg2 = f"Código: {cod_i}"
                msg3 = f"{descr_or_i}"
            ...
            data_inicio = datetime.strptime(data_ini, formato).date()

            sabados, domingos = self.definir_folgas(data_inicio, ultimo_data)
            dias_folga = sabados + domingos

            self.line_Folga.setText(str(dias_folga))
            self.line_TotalItens.setText(str(len(tabela_pra_tabela)))
            self.date_Final.setDate(ultimo_data)
        ...
        try:
            ...
            if lista_final:
                for i in lista_final:
                    ...
                    dados = (pontos, codis, descr, ref, um, qtde, entrega, conj, cod_pai, pacote)
    ...
    def calculo_7_manipula_previsao_pi(self, dados_previsao):
        try:
            ...
            dados_pi = extrair_tabela(self.table_PI)
            ...
            for i in dados_pi:
                ...
                msg1 = "Calculando Previsão de Entrega..."
                msg2 = f"Código: {cod_pi}"
                msg3 = f"{descr_pi}"
        ...
        try:
            ...
            if lista_nova_nova:
                ...
                if lista_nova_nova:
                    ...
                    lanca_tabela(self.table_PI, tab_ordenada)

            self.widget_Barra.setHidden(True)
        ...
        try:
            ...
            if not dados_extraidos:
                ...
                for dados_ex in dados_extraidos:
                    nivel, cod, descr, ref, um, qtde, entrega, conj, cod_pai, pacote = dados_ex
    ...
    def excel_total(self, caminho, nova_tabela):
        try:
            ...
            for i in nova_tabela:
                nivel, cod, descr, ref, um, qtde, entrega, conj, cod_pai, pacote = i

                dados = (nivel, cod, descr, ref, um, qtde, entrega, conj, cod_pai, pacote)
            ...
            if lista_nova:
                ...
                for d_ex in lista_nova:
                    nivel_ex, cod_ex, descr_ex, ref_ex, um_ex, qtde_ex, entr_ex, conj_ex, cod_pai_ex, pacote_ex = d_ex
                    nivius = int(nivel_ex)
                    codigu = int(cod_ex)
                    if cod_pai_ex:
                    ...
                    sheet.append([nivius, codigu, descr_ex, ref_ex, um_ex, qtde_e, entr_ex, conj_ex, cod_pai_ex_int,
                                  pacote_ex])
    ...
    def excel_pedido_interno(self, caminho_arquivo):
        try:
            ...
            headers = ["Emissão", "Nº PI", "Código", "Descrição", "Referência", "UM", "Qtde", "Previsão",
            ...
            if dados_tabela:
                for i in dados_tabela:
                    num_pi, codigo, descr, ref, um, qtde, limite, entrega, contagem = i
                    ...
                    if dados_interno:
                        ...
                        planilha.append([emi, num_pi_int, codigo_int, descr, ref, um, qtde_float,
                                         entrega, total_itens, contagem_int, porc_int])
