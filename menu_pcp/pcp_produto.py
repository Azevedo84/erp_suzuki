...
class TelaPcpProduto(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.layout_proprio()
        self.layout_tabela_producao(self.table_Producao)
        self.layout_tabela_compras(self.table_Compra)
        self.layout_tabela_consumo(self.table_Consumo)
        self.layout_tabela_venda(self.table_Venda)
    ...
    def layout_proprio(self):
        try:
            ...
            cor_widget(self.widget_Consumo)
            cor_widget(self.widget_Compras)
            cor_widget(self.widget_Vendas)
            cor_widget(self.widget_Estrutura)
            cor_widget(self.widget_Mov)
            cor_widget(self.widget_Producao)
            cor_widget(self.widget_Usado)
    ...
    def layout_tabela_producao(self, nome_tabela):
        try:
    ...
    def layout_tabela_compras(self, nome_tabela):
        try:
            layout_cabec_tab(nome_tabela)
    ...
    def layout_tabela_venda(self, nome_tabela):
        try:
            layout_cabec_tab(nome_tabela)
    ...
    def layout_tabela_usado(self, nome_tabela):
        try:
            layout_cabec_tab(nome_tabela)
    ...
    def layout_tabela_mov(self, nome_tabela):
        try:
            layout_cabec_tab(nome_tabela)
    ...
    def lanca_dados_produto_manual(self):
        try:
            ...
            cur.execute(f"SELECT prod.id, prod.descricao, "
                        ...
                        f"COALESCE(tip.tipomaterial, '') as tips "
            ...
            if detalhes_produto:
                id_prod, descr, compl, ref, um, local, ncm, saldo, embal, kg_mt, conjunto, tipo = detalhes_produto[0]
                ...
                mensagem_alerta("Este cadastro de produto não existe!")
    ...
    def limpa_tabelas(self):
        try:
            ...
            limpa_tabela(self.table_Producao)
            limpa_tabela(self.table_Estrutura)
            limpa_tabela(self.table_Mov)
            limpa_tabela(self.table_Venda)
            limpa_tabela(self.table_Consumo)
            limpa_tabela(self.table_Compra)
            limpa_tabela(self.table_Usado)
            ...
            if op_abertas:
                ...
                for dados_op in op_abertas:
                    ...
                    for dads in itens_estrutura:
                        ...
                        cursor = conecta.cursor()
                        cursor.execute(f"SELECT max(prodser.id_materia), sum(prodser.qtde_materia)as total "
                                       f"FROM materiaprima as mat "
                                       f"INNER JOIN produto as prod ON mat.produto = prod.id "
                                       f"INNER JOIN produtoos as prodser ON mat.id = prodser.id_materia "
                                       f"where prodser.numero = {op} and mat.id = {ides} "
    ...
    def manipula_dados_tabela_usado(self, cod_prod):
        try:
            ...
            cursor = conecta.cursor()
            cursor.execute(f"SELECT id, mestre, quantidade "
                           f"from materiaprima where codigo = {cod_prod};")
            tabela_estrutura = cursor.fetchall()
            for i in tabela_estrutura:
                ides_mat, mestre, qtde = i

                cursor = conecta.cursor()
                cursor.execute(f"SELECT codigo, descricao, COALESCE(obs, '') as ref, unidade, "
                               f"COALESCE(obs2, '') as obs "
                               f"FROM produto "
                               f"where id = {mestre};")
                select_prod = cursor.fetchall()
    ...
    def manipula_dados_tabela_consumo(self, cod_prod):
        try:
            ...
            cursor = conecta.cursor()
            cursor.execute(f"SELECT id, mestre from materiaprima where codigo = {cod_prod};")
            dados_estrut = cursor.fetchall()
            for i_estrut in dados_estrut:
                ides_mat, mestre = i_estrut
                ...
                if op_abertas:
                    for ii in op_abertas:
                        ...
                        cursor = conecta.cursor()
                        cursor.execute(f"SELECT mat.id, prod.codigo, "
                                       f"((SELECT quantidade FROM ordemservico where numero = {num_op}) * "
                                       f"(mat.quantidade)) AS Qtde "
                                       f"FROM materiaprima as mat "
                                       f"INNER JOIN produto as prod ON mat.produto = prod.id "
                                       f"where mat.id = {ides_mat};")
                        ...
                        if select_estrut:
                            ...
                            cursor.execute(f"SELECT max(prod.codigo), max(prod.descricao), "
                                           ...
                                           f"INNER JOIN produto as prod ON mat.produto = prod.id "
                                           f"INNER JOIN produtoos as prodser ON mat.id = prodser.id_materia "
                                           f"where mat.mestre = {mestre} "
                                           f"and prodser.numero = {num_op} and mat.id = {id_mat} "
                                           f"group by prodser.id_materia;")
