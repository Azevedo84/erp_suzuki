...
class TelaOpConsumir(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        validator = QtGui.QIntValidator(0, 123456, self.line_Cod_Manu)
        locale = QtCore.QLocale("pt_BR")
        validator.setLocale(locale)
    ...
    def verifica_vinculo_materia(self):
        try:
            id_os, numero_os, data_emissao, status_os, produto_os, qtde_os = self.dados_os()
    ...
    def verifica_dados_os(self):
        try:
            ...
            cursor.execute(f"SELECT materiaprima.id, produto.codigo, produto.descricao, "
                           ...
                           f"COALESCE(produto.localizacao, ''), produto.quantidade "
                           f"FROM materiaprima "
    ...
    def lanca_dados_os(self):
        try:
            ...
            cur.execute(f"SELECT codigo, descricao, COALESCE(obs, ' ') as obs, "
                        f"COALESCE(descricaocomplementar, ' ') as compl, unidade "
                        f"FROM produto where id = {produto_os};")
            detalhes_produtos = cur.fetchall()

            codigo_id, descricao_id, referencia_id, complementar_id, unidade_id = detalhes_produtos[0]
    ...
    def dados_os(self):
        try:
            numero_os_line = self.line_Num_OP.text()
            cur = conecta.cursor()
            cur.execute(f"SELECT id, numero, datainicial, status, produto, quantidade "
                        f"FROM ordemservico where numero = {numero_os_line};")
            extrair_dados = cur.fetchall()
            id_os, numero_os, data_emissao, status_os, produto_os, qtde_os = extrair_dados[0]

            return id_os, numero_os, data_emissao, status_os, produto_os, qtde_os
            ...
            if (event.type() == QtCore.QEvent.MouseButtonDblClick and
                ...
                for itens in extrai_total:
                    ...
                    if not cod_os and saldo_float < qtde_est_float and id_mat_selecao == id_mat:
                        item_sem_saldo = item_sem_saldo + 1
                        lista_est = (id_mat, cod_est, descr_est, ref_est, um_est, qtde_est, local, saldo)
                        tabela_estrutura.append(lista_est)
                        ...
                        item_ja_lancado = item_ja_lancado + 1
                        lista_est = (id_mat, cod_est, descr_est, ref_est, um_est, qtde_est, local, saldo)
                        tabela_estrutura.append(lista_est)
    ...
    def verifica_estrutura_produto_manu(self):
        try:
            ...
            cursor.execute(f"SELECT materiaprima.id, produto.codigo, produto.descricao, "
                           ...
                           f"FROM materiaprima "
                           f"INNER JOIN produto ON materiaprima.produto = produto.id "
                           f"where mestre = {produto_os} AND produto.codigo = {codigo_produto} "
                           f"ORDER BY produto.descricao;")
    ...
    def manipulando_dados_manu(self):
        try:
            ...
            cursor.execute(f"SELECT materiaprima.id, produto.codigo, produto.descricao, "
                           ...
                           f"FROM materiaprima "
                           f"INNER JOIN produto ON materiaprima.produto = produto.id "
                           f"where mestre = {produto_os} AND produto.codigo = {codigo_manu} "
                           f"ORDER BY produto.descricao;")
            item_estrutura = cursor.fetchall()
    ...
    def manipulando_dados_subs(self):
        try:
            ...
            cursor.execute(f"SELECT materiaprima.id, produto.codigo, produto.descricao, "
                           ...
                           f"FROM materiaprima "
                           f"INNER JOIN produto ON materiaprima.produto = produto.id "
                           f"where mestre = {produto_os} AND produto.codigo = {codigo_subs} "
                           f"ORDER BY produto.descricao;")
            item_estrutura = cursor.fetchall()
                ...
                if not itens_para_excluir:
                    ...
                    for dado_exc in itens_para_excluir:
                        ...
                        cursor.execute(
                            ...
                            f"and codigo = {cod_os} "
                            f"and id_materia = {id_mat} "
                            f"and quantidade = {qtde_os} "
                            f"and qtde_materia = {qtde};")
                ...
                if not itens_para_adicionar:
                    ...
                    for dado_adi in itens_para_adicionar:
                        ...
                        cursor.execute(f"Insert into produtoos (ID, DATA, PRODUTO, QUANTIDADE, MESTRE, "
                                       f"funcionarios,  movimentacao,  NUMERO, CODIGO, ID_MATERIA, QTDE_MATERIA, "
                                       f"DATA_CONSUMO) "
