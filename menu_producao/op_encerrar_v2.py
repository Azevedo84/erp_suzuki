...
class TelaOpEncerrarV2(QMainWindow, Ui_MainWindow):
    ...
    def dados_os(self):
        try:
            ...
            id_os, numero_os, data_emissao, status_os, produto_os, qtde_os, obs, id_estrutura = extrair_dados[0]

            return id_os, numero_os, data_emissao, status_os, produto_os, qtde_os, obs, id_estrutura
    ...
    def select_total(self):
        try:
            ...
            cursor = conecta.cursor()
            cursor.execute(f"SELECT estprod.id, prod.codigo, prod.descricao, COALESCE(prod.obs, ''), "
                           f"COALESCE(prod.unidade, ''), "
                           f"CASE when produtoos.QTDE_ESTRUT_PROD is NULL then ((SELECT quantidade "
                           f"FROM ordemservico where numero = {numero_os}) * (estprod.quantidade)) "
                           f"else produtoos.QTDE_ESTRUT_PROD end as teste, "
                           f"produto.localizacao, prod.quantidade, "
                           ...
                           f"FROM estrutura_produto as estprod "
                           f"INNER JOIN produto prod ON estprod.id_prod_filho = prod.id "
                           f"LEFT JOIN produtoos ON estprod.id = produtoos.id_estrut_prod "
                           f"where estprod.id_estrutura = {id_estrut} ORDER BY prod.descricao;")
    ...
    def manipulando_dados_select(self):
        try:
            ...
            for itens_estrut in select_estrut:
                ...
                cursor = conecta.cursor()
                cursor.execute(f"SELECT COALESCE(sum(QTDE_ESTRUT_PROD), '') FROM produtoos WHERE numero = {numero_os} "
                               f"and id_estrut_prod = {id_mat_e};")
        ...
        try:
            ...
            for linha_est in range(linhas_est):
                ...
                cursor.execute(f"SELECT id, movimentacao, codigo FROM produtoos where numero = {num_op} "
                               f"and codigo = {cod_os} "
                               f"and id_estrut_prod = {id_mat_os} "
                               f"and quantidade = {qtde_os} "
                               f"and QTDE_ESTRUT_PROD = {qtde_est};")
            ...
            cursor.execute(f"UPDATE ordemservico SET "
                           ...
                           f"obs = '{obs_maiusculo}', LOCAL_OP = 'ENCERRADA' "
